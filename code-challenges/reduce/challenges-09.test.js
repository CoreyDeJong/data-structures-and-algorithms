...
const countNumberOfElements = (arr) => {
  return arr.reduce( function(accum, value, idx) {
    accum = idx+1;
    return accum;
  }, 0);
};
...
const returnNames = (arr) => {
  return arr.reduce( function(answersSoFar, value, index){ answerSoFar = value.name.push(value.name)
...
const calculateAverage = (arr) => {
  let total = arr.reduce(function (accum, value, idx){
    accum = accum+value;
    return accum;
  },0);
